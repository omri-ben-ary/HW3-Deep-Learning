...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    # ====== YOUR CODE: ======
    onehot_tensor = chars_to_onehot(text, char_to_idx).to(device)
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        h = None
        x = chars_to_onehot(start_sequence, char_to_idx).unsqueeze(dim=0).to(device=device, dtype=torch.float)
        for _ in range(n_chars - len(start_sequence)):
            ...
            x = chars_to_onehot(next_char, char_to_idx).unsqueeze(dim=0).to(device=device, dtype=torch.float)
            out_text += next_char 
    # ========================
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i in range(n_layers):
            ...
            self.layer_params.append({"wxz": w_xz, "whz": w_hz, "wxr": w_xr, "whr": w_hr, "wxg": w_xg, "whg": w_hg})
